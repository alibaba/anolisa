...
| 723 | `src/tools/command_risk.rs` | tools | Command risk assessment; near-threshold file (main baseline 697). The per-segment compound assessment fix (GH-1785) added 26 lines crossing the threshold. Split plan: extract compound aggregation and readonly-evidence helpers into `command_risk_compound.rs`. |
