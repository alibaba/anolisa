...
- Only risks that already exist on `main`/`HEAD` may be registered; new
  ...
  target. The automatic-compaction feature followed this: its process-spawn and
  reaping tests live in `slash/session/compact/tests.rs`, which is excluded
  from this scan as a test module, and exercise placeholder processes only.
