...
捕获始终要求所选后端支持完整快照。数据面路径由提供者声明的集成方式决定，不能只根据
是否存在提供者租约判断。由提供者持有检查点内容时，提供者必须实现
`DataPlaneCheckpoint`，并持有包含可写根文件系统和客户机内存的完整映像。提供者若声明
`daemon_managed_storage`，则由配置的存储提供程序捕获可写根文件系统，不需要实现检查点
扩展；标准文件提供者采用这条路径。Firecracker 和内置模拟后端支持完整捕获，
Bubblewrap 和其他进程后端尚不支持。不支持的组合会在静止后端或修改生命周期记录前返回
HTTP 501。
...
在由提供者持有内容的扩展路径上，Blaze 仍把后端制品保存在 `backend/` 下，而
`DataPlaneCheckpoint` 会在同一静止边界冻结包含可写根文件系统和客户机内存的完整
不可变映像。持久化清单保存与实现无关的引用和摘要，不要求存在
`storage/rootfs.snap`；管理接口响应不会返回这条所有权记录。
...
恢复要求目标是经过校验的完整检查点，且策略、镜像、后端和后端版本都与当前沙箱
完全一致，并具有兼容的后端恢复适配器。通过 `daemon_managed_storage` 捕获的检查点
要求存储支持恢复，即使活动租约来自构建时提供者也是如此。只有包含提供者不可变引用的
检查点才要求匹配的提供者实例实现 `DataPlaneCheckpoint`。不支持的组合会在停止当前
运行环境前返回 HTTP 501。
...
数据面路径同样由提供者声明的集成方式决定。由提供者持有休眠内容时，提供者必须实现
`DataPlaneSuspend`，沙箱必须具有已最终确认的活动租约，并支持客户机协议版本 1 的
`prepare_hibernate`、`reseed_rng` 和 `post_restore` 操作。提供者若声明
`daemon_managed_storage`，则改用配置的存储提供程序，不需要实现休眠扩展。
...
活动租约，而是保留不可变的休眠引用，并在每次恢复时准备新租约。两条路径都会保留
最近一次可恢复的映像或引用，直到下一次休眠替换它或销毁将其删除。守护进程重启后，
已经完成的休眠仍可恢复，但中断的休眠或恢复操作不会自动续做，而是以
