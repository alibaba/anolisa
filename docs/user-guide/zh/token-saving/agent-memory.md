...
| install.sh 报安装被 `security.installPolicy` 拦截 | OpenClaw 2026.6.5 及之后已无安装期扫描，`--dangerously-force-unsafe-install` 是 deprecated no-op，脚本不再传递 | 放宽 `security.installPolicy`——重跑脚本或设置 `AGENT_MEMORY_SAFE_INSTALL` 都无法覆盖该策略 |
