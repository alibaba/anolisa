...
- 若用户要求批量操作 → 使用 `check --all`（CLI 内部读取 `/etc/agent-sec/skill-security/ledger/config.json` 的 `skillDirs` 并展开 glob）
...
```bash
ls /etc/agent-sec/skill-security/ledger/keys/key.pub
```
...
1. 读取 `/etc/agent-sec/skill-security/ledger/config.json` 的 `scanners[]` 配置
