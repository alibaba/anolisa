...
| 文件 | 路径 | 权限 |
|------|------|------|
| 加密私钥 | `/etc/agent-sec/skill-security/ledger/keys/key.enc` | 0600 |
| 公钥 | `/etc/agent-sec/skill-security/ledger/keys/key.pub` | 0644 |
...
默认已包含三个内置目录：`~/.openclaw/skills/*`、`~/.copilot-shell/skills/*`、`/usr/share/anolisa/skills/*`。如需添加额外目录，创建或编辑 `/etc/agent-sec/skill-security/ledger/config.json`：
...
配置文件中的 `skillDirs` 会**追加**到默认目录之后（自动去重），无需重复声明默认目录。
...
| 路径 | 用途 |
|------|------|
| `/etc/agent-sec/skill-security/ledger/keys/key.enc` | 加密私钥 |
| `/etc/agent-sec/skill-security/ledger/keys/key.pub` | 公钥 |
| `/etc/agent-sec/skill-security/ledger/keys/keyring/` | 归档的历史公钥（密钥轮换后） |
| `/etc/agent-sec/skill-security/ledger/config.json` | 配置文件（skillDirs、scanners） |
