...
| 概念 | 说明 |
|------|------|
| **Manifest** | JSON 记录（`.skill-meta/latest.json`），包含文件哈希、扫描结果和数字签名；首次 `check` 创建的 baseline 可能尚未签名，`certify` 后会补签 |
...
agent-sec-cli skill-ledger certify /path/to/your-skill \
  --scanners skill-code-scanner,cisco-static-scanner
```
...
- **Phase 2**（快速扫描认证）：调用内置 `skill-code-scanner` 与 `cisco-static-scanner`，再签名写入 manifest
...
| `agent-sec-cli skill-ledger certify <dir> --scanners skill-code-scanner,cisco-static-scanner` | 执行快速扫描并签名写入 manifest |
