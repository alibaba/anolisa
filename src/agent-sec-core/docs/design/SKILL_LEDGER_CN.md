...
│          └──── .skill-meta/ ────────┘                  │
│                                                       │
│  /etc/agent-sec/skill-security/ledger/keys/                     │
...
/etc/agent-sec/skill-security/ledger/         # 系统配置
└── config.json                # 签名后端、skill 目录等偏好设置

/etc/agent-sec/skill-security/ledger/keys/   # 签名密钥与历史公钥存储（位于 skill 目录外部）
...
**密钥与 skill 分离**：签名私钥存储在 `/etc/agent-sec/skill-security/ledger/keys/` 而非 `.skill-meta/` 内。即使攻击者完全控制 skill 目录，也无法伪造有效签名。密钥作为系统级安全资产随 skill-security 配置集中管理，私钥文件通过 `0600` 权限保护。
...
**防御原则**：签名权与文件访问权分离。签名私钥位于 `/etc/agent-sec/skill-security/ledger/keys/`（skill 目录外部），即使完全控制 skill 目录也无法伪造有效签名。
...
通过 `/etc/agent-sec/skill-security/ledger/config.json` 配置：
```jsonc
{
...
**合并策略**：配置中的 `skillDirs` 为**追加合并**（additive merge）——默认目录在前，配置目录在后，自动去重。无需重复声明默认目录。其余配置项（如 `signingBackend`）仍为覆盖合并。
...
4. 写入 /etc/agent-sec/skill-security/ledger/keys/key.enc（mode 0600）
5. 写入公钥 → /etc/agent-sec/skill-security/ledger/keys/key.pub
...
生成 Ed25519 密钥对，写入 `/etc/agent-sec/skill-security/ledger/keys/key.enc`（mode 0600）。默认不加密（明文种子）；指定 `--passphrase` 或设置 `SKILL_LEDGER_PASSPHRASE` 环境变量时使用 scrypt + AES-256-GCM 加密。输出公钥指纹。
...
列出内置默认及 `/etc/agent-sec/skill-security/ledger/config.json` 中注册的所有扫描器，包括名称、调用类型、结果解析器和启用状态。用于发现 `certify --scanner` 可用的扫描器名称。
