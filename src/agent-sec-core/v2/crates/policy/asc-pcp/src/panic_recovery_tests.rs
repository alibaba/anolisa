...
fn expected_failure(mut record: ReconcileRecord, registered: bool) -> ReconcileRecord {
    ...
    record.runtime.last_error = Some(Failure::new(
...
fn panic_failure_waits_for_storage_repair_without_repeating_io() {
    ...
    running.binding.status = BindingStatus::Applying;
    running.runtime.last_error = None;
...
fn panic_during_completion_preserves_success_and_never_repeats_create() {
    for point in ["before_finish", "after_finish"] {
        ...
        expected.runtime.last_error = None;
...
fn delete_panic_preserves_unknown_target_and_marks_delete_failed() {
    ...
    input.binding.status = BindingStatus::PendingDelete;
