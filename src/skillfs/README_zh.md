...
- `--notify-socket <PATH>` 将 debounce 后的 skill mutation 通知发给外部 daemon。
  ...
  `--notify-auth-key-file <PATH>` 可以为容器化 sec-core peer 启用双向 HMAC
  认证，同时保持 notify v2 业务 payload 不变。
...
- `--control-socket <PATH>` 在恰好配置一种 peer mode 时启动可信 Unix socket
  ...
  `--trusted-peer-key-file <PATH>` 启用显式容器 HMAC profile，并要求显式 socket
  path。可信 peer 可通过 `meta.writeActivation`、`meta.setActivationXattr` 等方法
  写 activation JSON 或 xattr。
