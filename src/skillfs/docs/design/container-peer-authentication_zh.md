...
开发分支已经实现 SkillFS 和 agent-sec-core 认证原语、control 与 notify wiring、
fail-closed 配置门禁、文档和各语言内部的回归测试。固定 proof vector 用于锁定
Rust/Python 字节契约。

真正的 Rust-to-Python、Python-to-Rust 独立容器 fixture、security-integrated Pod
profile 和 ACK 证据仍未完成。在这些跨组件和部署验收项通过前，本 issue 必须保持
open。
...
非可信 workload 可能知道 socket 路径；负向测试还会故意让它看到 runtime
volume。只要没有 Secret，它仍必须认证失败，即使使用与可信 peer 相同的 UID、
GID、进程名或 executable basename。
...
agent-sec-core 从同一个绝对路径、no-follow、有大小上限、权限严格的普通文件
加载 secret。UID/GID 仍可作为附加约束，但不能被当作 container identity。
...
完成 `auth.ok` 后，双方原样交换现有业务 NDJSON request/response。这能避免跨
语言 JSON canonicalization，并保持 control schema v1 和 notify schema v2 不变。
...
- 定义兼容的 challenge/proof frame、大小限制、timeout、domain string 和固定的
  跨语言 test vector。
...
在 `src/agent-sec-core` 下执行针对性的 formatter、lint、type 和 pytest 检查，再
运行独立容器 fixture。该 fixture 遇到认证或 namespace 条件不可用时必须失败，
不能 skip。

必须覆盖的负向场景包括 missing、empty、short、oversized、symlinked、权限过宽
和 owner 错误的 secret file；错误、畸形、过期或 replayed proof；认证 timeout；
UID/GID 不匹配；向 HMAC mode 发送 plain request；以及可以访问 socket、使用相同
UID 但没有 Secret 的非可信 peer。
