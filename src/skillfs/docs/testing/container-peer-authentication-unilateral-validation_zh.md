...
| workload / attacker | 普通 I/O 使用不同 UID；socket 负向测试再用 UID 0 重复 | 不挂载 | read-write，传播获得 | 默认不挂载；负向测试故意挂载 | 不挂载 |
...
首版 profile 要求 auth file owner 与进程 effective UID 相同。未来 SkillFS 和
sec-core 使用不同 UID 时，应为两个 container 分别 stage 包含相同 raw bytes 的
私有副本，并在各自 container 中挂载到配置的绝对路径。
...
Replay 必须观察到新 nonce，并且不能收到 `auth.ok`。Slow sender 必须在七秒内被
断开。最后的 trusted request 必须通过，证明单请求 listener 已恢复。
...
| C7-U03 | 每条连接 nonce 不同，并拒绝 replay |
