...
| workload / attacker | different UID for normal I/O; repeat negative socket tests with UID 0 | absent | read-write, propagated | absent normally; intentionally present for negative tests | absent |
...
The initial profile requires the auth file to be owned by each process's
effective UID. If future SkillFS and sec-core containers use different UIDs,
stage separate private copies containing the same raw bytes and mount them at
the configured absolute path in each container.
...
export C7_NOTIFY=/run/anolisa/sec-core/daemon.sock
...
- `skills/weather/SKILL.md` is readable through the workload's propagated
  view; and
- the control socket is mode `0600` in a private runtime directory.
...
### 5.2 Replay and slow partial frames
...
The replay must observe a new nonce and receive no `auth.ok`. The slow sender
must be disconnected within seven seconds. The final trusted request must pass,
proving the single-request listener recovered.
...
| C7-U03 | nonce differs across connections and replay is rejected |
