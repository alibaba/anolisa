...
reads a sensitive credential and then connects to an untrusted public endpoint.
...
- `trusted`: configured model endpoints, internal services, and approved package
  sources.
- `unknown_public`: a public destination without an explicit trust decision.
...
and connects to a public network destination within five minutes,
...
- Sensitive read followed by an unknown public destination: audit until the
...
3. Reading the test credential followed by an unknown public destination creates
   a complete risk case in audit mode while allowing the operation.
4. The same scenario in enforce mode returns `EPERM` from the real kernel hook.
...
7. Enable deny only for the accepted unknown-public-destination scenario.
