...
**Goal:** Build an AgentSight-owned credential-exfiltration audit loop that receives ActPlane security facts, correlates an explainable risk case, and progresses from observe to audit to a real kernel denial without requiring AgentSecCore.

**Architecture:** Complete the foundational UDS enforcer in `2026-07-15-actplane-enforcement-implementation-plan.md` first. This plan extends that boundary with stable security events, an AgentSight-local store and case correlator, the existing security Dashboard, and one product policy translated only inside the ActPlane adapter.

**Tech Stack:** Rust 2024, bounded NDJSON over Unix domain sockets, SQLite/rusqlite, Actix Web, React 18, TypeScript, Vitest, BPF-LSM, and official ActPlane pinned to `a62e5d9d96f91101cda019519053e950d532380a`.
...
- Dashboard gates are `npm run typecheck` and `npm test -- --run` from `src/agentsight/dashboard`.
...
- Server integration: `src/server/{mod.rs,handlers.rs}`.
- Existing Dashboard surface: `dashboard/src/pages/SecurityObservabilityPage.tsx`, `dashboard/src/pages/security/`, and `dashboard/src/utils/apiClient.ts`.
...
- Produces: `SecurityEvent`, `SecurityEventKind`, `EventIdentity`, `FileAction`, `TaintTransition`, `NetworkAction`, `PolicyDecision`, `EnforcementStateEvent`, `CredentialExfiltrationPolicy`, and `PolicyMode`.
...
`CredentialExfiltrationPolicy` contains policy ID, numeric revision, redacted source patterns, trusted endpoints, taint label, TTL seconds, and mode. Validation rejects empty IDs, revision zero, empty sources, empty taint, TTL outside `1..=86400`, and more than 1,024 source or destination patterns.
...
### Task 5: Serve local security data without AgentSecCore

**Files:**
- Modify: `src/agentsight/src/server/mod.rs`
- Modify: `src/agentsight/src/server/handlers.rs`
- Modify: `src/agentsight/src/bin/cli/serve.rs`
- Test: `src/agentsight/src/server/handlers.rs`

**Interfaces:**
- Consumes: `SecurityStore` and `SecurityCoordinator`.
- Produces: local `/api/security/*` and `/api/audit/cases/*` responses with no UDS call to AgentSecCore.

- [ ] **Step 1: Replace daemon-proxy tests with failing local-store tests**

```rust
#[actix_web::test]
async fn security_status_is_local_ready_without_agent_sec_socket() {
    let app = test_app_with_security_events(vec![]).await;
    let request = test::TestRequest::get().uri("/api/security/status").to_request();
...
- [ ] **Step 2: Verify focused server tests fail**

Run `cd src/agentsight && cargo test -p agentsight server::handlers::tests::security_`.

Expected: FAIL because status still depends on the AgentSecCore socket and case routes are missing.

- [ ] **Step 3: Put the local store in `AppState` and remove runtime proxying**

Replace `SecurityObservabilityConfig` with:
...
Keep existing `/api/security/summary`, `/events`, `/events/count-by`, event detail, sessions, runs, and timeline paths so the current Dashboard remains compatible. Implement them from `SecurityStore`; set response `meta.source="agentsight"`. Add:
...
The review body accepts only `confirmed`, `false_positive`, `accepted_risk`, or `resolved`. It updates review state but never publishes a policy revision. Preserve `src/agent_sec/` as an unused compatibility module in this task; remove it only in a separate deprecation change after confirming no external Rust API consumer.
...
cargo fmt --all -- --check
cargo clippy -p agentsight --all-targets -- -D warnings
cargo test -p agentsight server::handlers::tests::security_
cargo test -p agentsight
```

Expected: local security APIs pass without an AgentSecCore daemon or socket.

- [ ] **Step 5: Commit**

```bash
git add src/agentsight/src/server src/agentsight/src/bin/cli/serve.rs
git commit -m "feat(sight): serve local security audit"
```

---

### Task 6: Adapt the existing security Dashboard to local risks and cases

**Files:**
- Modify: `src/agentsight/dashboard/src/utils/apiClient.ts`
- Modify: `src/agentsight/dashboard/src/pages/SecurityObservabilityPage.tsx`
- Modify: `src/agentsight/dashboard/src/pages/security/types.ts`
- Modify: `src/agentsight/dashboard/src/pages/security/OverviewTab.tsx`
- Modify: `src/agentsight/dashboard/src/pages/security/EventDetailDrawer.tsx`
- Create: `src/agentsight/dashboard/src/pages/security/RiskCaseDetail.tsx`
- Modify: `src/agentsight/dashboard/src/test/SecurityObservabilityPage.test.tsx`

**Interfaces:**
- Consumes: Task 5 API envelopes.
- Produces: local-ready status, risk-case list/detail, ordered evidence, and review actions.

- [ ] **Step 1: Add failing local-ready and case-detail tests**

```tsx
it('renders AgentSight local security without an AgentSec daemon', async () => {
  mockSecurityStatus({ state: 'local_ready', data: { source: 'agentsight' } });
  mockOneCredentialExfiltrationCase();
  render(<SecurityObservabilityPage />);
  expect(await screen.findByText('疑似凭据外传')).toBeInTheDocument();
  expect(screen.queryByText('agent-sec daemon is unavailable')).not.toBeInTheDocument();
});

it('distinguishes audit allow from a confirmed kernel block', async () => {
  mockAuditAndBlockedCases();
  render(<SecurityObservabilityPage />);
  expect(await screen.findByText('规则命中，已放行')).toBeInTheDocument();
  expect(screen.getByText('内核已拦截')).toBeInTheDocument();
});
```
...
npm test -- --run SecurityObservabilityPage.test.tsx
```

Expected: FAIL because only `daemon_reachable` is treated as available and risk-case UI is missing.

- [ ] **Step 3: Add local API types and interactions**

Add `local_ready` to `SecurityApiState`, `SecurityRiskCase`, `SecurityRiskCaseDetail`, and `SecurityReviewStatus`. Add `fetchSecurityCases`, `fetchSecurityCase`, and `reviewSecurityCase`. Treat `local_ready` as available; show the data source as AgentSight, not AgentSecCore.

`RiskCaseDetail` renders four explicit answers: what happened, observed block result, reason, and impact. Render evidence in source-read, taint, network-sink, decision order. Use distinct copy for audit allow, requested-but-failed deny, confirmed kernel block, and enforcer unavailable. `创建例外` produces a proposal modal only; it does not call a policy-publish API.
...
Expected: typecheck, all Vitest tests, and the embedded production build pass.

- [ ] **Step 5: Commit**

```bash
git add src/agentsight/dashboard src/agentsight/frontend-dist
...
fn trusted_endpoint_becomes_an_unless_exception() {
    let policy = fixture_policy_with_trusted_endpoint("llm.example.com:443");
    ...
    assert!(dsl.contains("llm.example.com:443"));
}
...
Generate deterministic DSL with sorted source and trusted-endpoint patterns. Escape only characters accepted by the pinned parser and reject newline, NUL, and quote injection before compilation. Use ActPlane source rules to add the configured taint label, a connect sink rule, and supported exception semantics. If the pinned upstream revision cannot express trusted endpoint exceptions in one policy, compile a trusted allow rule with higher priority and document that exact ordering in the adapter test; do not move DSL logic into AgentSight.

Convert raw provenance into ordered `FileAction`, `TaintTransition`, `NetworkAction`, and `PolicyDecision` events. Preserve raw kernel operation, target, label name, origin PID/time, rule ID/reason, domain, and observed blocked/killed bits. Redact configured home prefixes before publishing.
...
### Task 8: Prove observe, audit, enforce, expiry, and restart on real Linux
...
assert_json '.data.mode == "enforce" and .data.blocked == true and .data.errno == "EPERM"'
assert_json '.data.evidence | map(.event_type) == ["file_action","taint_transition","network_action","policy_decision"]'
```

The script also verifies a trusted endpoint is allowed, a child process inherits taint, an unrelated process remains unaffected, TTL expiry restores access, and enforcer restart reconciliation never reports unknown state as enforced.
...
Expected: observe, audit, deny, trusted exception, isolation, expiry, and restart assertions pass. The deny scenario returns real `EPERM` and the API reports `blocked=true`.

- [ ] **Step 4: Measure the release budget**

Record idle RSS/CPU before and after enabling the enforcer and measure P99 decision latency over at least 10,000 allowed and 10,000 denied fixture operations. Fail the acceptance script when added idle RSS is >=30 MiB, added idle CPU is >=1%, or P99 overhead is >=2 ms. Record event drop counters and require zero loss for the acceptance load.

- [ ] **Step 5: Update bilingual usage docs and measured design status**

Document the internal enforcer, BPF-LSM/BTF requirements, observe/audit/enforce semantics, default evidence exclusions, fail-open attachment behavior, and the exact supported credential policy. Mark only measured capabilities as implemented and move unimplemented items to later planning.
...
- [ ] No AgentSecCore socket or process is required for `/api/security/*`.
...
- [ ] Observe creates metrics only; audit creates an unblocked case; enforce records a real kernel block.
...
- [ ] Dashboard typecheck, Vitest, and embedded build pass.
- [ ] Real Linux acceptance returns `EPERM` and reports `blocked=true`.
