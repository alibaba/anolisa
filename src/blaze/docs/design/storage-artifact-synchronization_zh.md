...
已经提交的检查点目录和一个 HEAD 引用。发布过程会校验必需的后端状态、内存和
可写根制品，记录大小与 SHA-256 摘要，同步文件和目录，再通过不替换已有目标的
重命名发布检查点。只有发布已经持久化后，才会原子更新 HEAD。查询历史前会重新
打开并校验已经提交的清单和制品，再报告历史与 HEAD 可达性。
