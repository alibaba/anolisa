...
`POST /v1/sandboxes/{id}/checkpoint` 要求所选后端支持完整快照。数据面处理由
提供者声明的集成方式决定，不能只根据是否存在提供者租约判断。可选的
`DataPlaneCheckpoint` 扩展持有包含可写根文件系统和客户机内存的完整映像。提供者若
声明 `daemon_managed_storage`，则由配置的存储提供程序捕获根文件系统，不需要实现该
扩展；标准文件提供者采用这条路径。不支持的组合会在静止后端或改变沙箱状态前返回
HTTP 501。新捕获发布检查点格式 3；格式 1 和 2 在原后端与存储要求仍满足时继续可读。
...
的那一份状态重新运行。这个接口要求后端具有兼容的恢复适配器。通过
`daemon_managed_storage` 捕获的检查点要求存储支持恢复，即使活动租约来自构建时
提供者也是如此。只有包含提供者不可变引用的检查点才要求当前构建时提供者实现
`DataPlaneCheckpoint`，并与记录的提供者身份一致。条件不满足时，Blaze 不改动沙箱的
任何状态，直接返回 HTTP 501。
...
对于由提供者持有内容的检查点，Blaze 会在旧后端仍然运行时，根据不可变引用准备一份
...
休眠用于在暂时不需要一个沙箱时释放它占用的后端运行资源，同时把运行状态完整保存
下来，之后再原样恢复运行。只有当运行中的后端支持完整快照捕获，并且配置的适配器能够
恢复相同的后端版本时，休眠才可用。由提供者持有休眠内容时，提供者还必须实现
`DataPlaneSuspend`、具有已最终确认的租约，并支持客户机协议版本 1 的
`prepare_hibernate`、`reseed_rng` 和 `post_restore` 操作。提供者若声明
`daemon_managed_storage`，则改用配置的存储提供程序，不需要实现休眠扩展。这些兼容性
检查都在改动生命周期记录之前完成，因此不满足条件的组合会让沙箱保持运行。至于如何
把工作负载带到一致的停止点，则交给后端的“捕获前静止”钩子：其默认行为是暂停后端，
而捕获操作自身会停止工作负载的后端会覆盖该钩子，因而无需单独支持暂停。
...
在由提供者持有内容的扩展路径上，提供者会在后端快照采用的同一静止边界冻结包含可写
根文件系统和客户机内存的完整不可变映像。后端停止后，Blaze 会停止并释放活动租约；
已休眠沙箱只保留不可变的休眠引用。
...
守护进程管理的路径会在休眠期间保留存储空间。由提供者持有内容的扩展路径不会保留
活动租约，并在每次恢复运行时准备新租约。两条路径都会保留最近一次可恢复的映像或
引用，直到下一次休眠替换它，或者显式销毁将其删除。守护进程重启后不会自动续做中断
的休眠或恢复操作。
