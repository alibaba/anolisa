...
- Status: leaf contract foundation implemented; Phase 0 freeze remains open
...
events, runtime messages, and capability requests defined here. Complete schema
and golden-fixture corpora, reducer property matrices, integrated coordinator
ports, and bridges remain Phase 0/1 work.
...
- Implementing the Gateway daemon, database, Runtime Supervisor, or ACP bridge.
...
The implementation worktree adds the neutral leaf types but no ACP dependency,
`TaskCoordinator`, Task event store, Outbox, or behavioral Runtime Port.
...
enum AgentRuntimeCommand {
    ...
    Prompt { run_id: RunId, input: Vec<ContentPart> },
    ResolvePermission { request_id: RequestId, decision: RuntimePermissionDecision },
    Cancel { run_id: RunId, cause: CancelReason },
...
`cosh.runtime.command`, and `cosh.runtime.event`, each starting at schema
version `1`. Schema version is unrelated to ACP `protocolVersion`.
