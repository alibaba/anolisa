...
_cosh_should_intercept_missing_path() {
  ...
  # Literal-word whitelist (review round 7, generalizing the round-6 glob
  # veto). The raw $BUFFER token has seen no lexing or expansion yet:
  # quoting, escaping, brace/parameter/command expansion and globbing can
  # all rewrite the word before zsh resolves it, so probing the literal
  # text proves nothing about the executed command. Instead of
  # enumerating dangerous characters, only a word made purely of
  # non-syntax characters (alphanumerics, `_./-`, and multibyte text) may
  # continue; any other ASCII character fails open to the native path.
  local _cosh_stripped="${first_word//[A-Za-z0-9_.\/-]/}"
  if [[ -n "$_cosh_stripped" && "$_cosh_stripped" == *[\ -~]* ]]; then
    return 1
  fi
  # Mirror zsh command-word resolution (review round 6). This wrapper runs
  # BEFORE zsh parses the line (unlike bash's hook, which fires after
  # resolution already failed), so the gate must reproduce the resolution
  # order zsh itself would apply to a slash-bearing word: aliases and
  # functions resolve before any path lookup — whence -w consults zsh's
  # own resolver, so anything it resolves stays native.
  if builtin whence -w -- "$first_word" >/dev/null 2>&1; then
    return 1
  fi
...
# nested-function limit. The in-progress flag routes the first re-entrant
# call to the saved accept-line target (preserving a user accept-line
# wrapper in the chain) and any deeper re-entry to the builtin; the
# always blocks clear the flags on every exit path.
...
_cosh_accept_line() {
  if [[ "${_COSH_DISPATCH_IN_PROGRESS:-0}" == 1 ]]; then
    # Re-entrant NAMED accept-line from the delegated widget: the native
    # chain is user widget -> user's accept-line target, so the first
    # re-entry reaches the saved alias (review round 6 — the builtin
    # would skip a user accept-line wrapper). A second-level re-entry
    # (the saved target itself calling the NAMED accept-line) terminates
    # on the builtin to keep the chain finite.
    if [[ "${_COSH_REENTRY_IN_PROGRESS:-0}" == 1 || "${_COSH_HAS_ORIG_ACCEPT_LINE:-0}" != 1 ]]; then
      zle .accept-line
      return
    fi
    _COSH_REENTRY_IN_PROGRESS=1
    {
      zle _cosh_orig_accept_line
    } always {
      _COSH_REENTRY_IN_PROGRESS=0
    }
    return
  fi
...
_cosh_claim_submit_key() {
  ...
  [[ -n "${_COSH_SUBMIT_KEY_WIDGETS[${keymap}:${key}]:-}" ]] && return 0
  _COSH_SUBMIT_KEY_WIDGETS[${keymap}:${key}]="$widget"
}
# Claim set: main, vicmd, and every user-defined keymap. zsh's builtin
# keymap set is closed (emacs/viins/vicmd/viopp/visual plus the modal maps
# isearch/command/.safe/menuselect/listscroll, and the main alias), so any
# other name reported by `bindkey -l` was created by the user — e.g. a map
# selected from zle-line-init via `zle -K` (review round 8). The modal
# builtins stay unclaimed because their submit keys do not mean "accept a
# command line" (accept-search, menu accept, execute-named-cmd).
# `bindkey -l` is split per line: keymap names may carry spaces (review
# round 9).
typeset -a _cosh_claim_keymaps
_cosh_claim_keymaps=(main vicmd)
for _cosh_keymap in ${(f)"$(builtin bindkey -l 2>/dev/null)"}; do
  case "$_cosh_keymap" in
    main|emacs|viins|vicmd|viopp|visual|isearch|command|.safe|menuselect|listscroll) ;;
    *) _cosh_claim_keymaps+=("$_cosh_keymap") ;;
  esac
done
# Two-phase mount (review round 9): phase 1 captures every keymap's
# original binding while no physical map has been touched — an aliased
# keymap therefore reads the true widget of the map it shares, with no
# scan-order dependence and no mirror heuristics. Phase 2 rebinds every
# captured key; rebinding a shared physical map twice is idempotent.
for _cosh_keymap in "${_cosh_claim_keymaps[@]}"; do
  for _cosh_submit_key in $'\r' $'\n'; do
    _cosh_claim_submit_key "$_cosh_keymap" "$_cosh_submit_key"
  done
done
# Table keys are "<keymap>:<key>" with a single-character key; split by
# fixed length from the tail — keymap names may contain ":" themselves.
for _cosh_claim_entry in "${(@k)_COSH_SUBMIT_KEY_WIDGETS}"; do
  builtin bindkey -M "${_cosh_claim_entry%??}" -- "${_cosh_claim_entry[-1]}" \
    _cosh_accept_line 2>/dev/null || true
done
unset _cosh_keymap _cosh_submit_key _cosh_claim_keymaps _cosh_claim_entry
