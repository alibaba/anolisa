...
_cosh_is_slash_control_candidate() {
  local command="$1"
  case "$command" in
    /about|/agent|/allow|/answer|/approval-mode|/approve|/audit|/auth|/cancel|/clear|/config|/copy|/debug|/deny|/details|/explain|/extensions|/health|/help|/hooks|/mcp|/mode|/new|/plan|/recommendations|/resume|/select|/send-to-shell|/session|/shell|/skills|/stats|/status)
      return 0
      ;;
  esac
  return 1
...
for _cosh_sc in about agent allow answer approval-mode approve audit auth cancel clear config copy debug deny details explain extensions health help hooks mcp mode new plan recommendations resume select send-to-shell session shell skills stats status; do
