...
_cosh_should_intercept_unknown() {
  ...
  case "$command" in
    /agent|/allow|/answer|/approval-mode|/approve|/audit|/auth|/cancel|/clear|/config|/copy|/debug|/deny|/details|/explain|/extensions|/health|/help|/hooks|/mode|/select|/send-to-shell|/shell|/skills)
      printf '%s' "slash"
      return 0
      ;;
  esac
...
for _cosh_sc in agent allow answer approval-mode approve audit auth cancel clear config copy debug deny details explain extensions health help hooks mode select send-to-shell shell skills; do
  functions[/$_cosh_sc]=':'
done
unset _cosh_sc
